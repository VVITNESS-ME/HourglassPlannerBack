...
import com.myweapon.hourglass.statistics.dto.BurstTimeByCategories;
import com.myweapon.hourglass.statistics.dto.TotalBurstByDay;
import com.myweapon.hourglass.statistics.dto.TotalBurstByMonth;
import com.myweapon.hourglass.statistics.entity.StudyStaticsView;
...
public interface StudyStaticsViewRepository extends JpaRepository<StudyStaticsView,Long> {
    ...
    @Query("select new com.myweapon.hourglass.statistics.dto.TotalBurstByDay(s.end,sum(s.burstTime))" +
    ...
    @Query("select new com.myweapon.hourglass.statistics.dto.TotalBurstByMonth(s.end,sum(s.burstTime))" +
            "from StudyStaticsView s " +
            "where s.userId = :userId and s.end between :start and :end " +
    ...
    @Query(value = "select new com.myweapon.hourglass.statistics.dto.BurstTimeByCategories( s.categoryName" +
            ",sum(s.burstTime)" +
            ",s.color )" +
            "from StudyStaticsView s " +
            "where s.userId = :userId and s.end between :start and :end " +
