...
import com.myweapon.hourglass.webchat.domain.Room;
import com.myweapon.hourglass.webchat.domain.RoomService;
import com.myweapon.hourglass.webchat.domain.WebSocketMessage;
...
public class SignalSocketHandler extends TextWebSocketHandler {
    ...
    private final Logger logger = LoggerFactory.getLogger(this.getClass()); // 로그 기록을 위한 Logger
    private final ObjectMapper objectMapper = new ObjectMapper(); // JSON 변환을 위한 ObjectMapper

    // session id to room mapping
    private Map<String, Room> sessionIdToRoomMap = new HashMap<>(); // 세션 ID와 방 정보를 매핑하기 위한 Map
